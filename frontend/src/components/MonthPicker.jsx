  ...
  return (
    <div className="flex items-center gap-1 bg-surface border border-border rounded-md px-2 py-1.5 select-none">
      <button
        ...
        className="w-6 h-6 flex items-center justify-center text-muted hover:text-fg rounded transition-colors"
      >‹</button>
      <span className="text-sm font-medium text-fg min-w-[130px] text-center">
        {MONTHS[month - 1]} {year}
      </span>
      <button
        onClick={() => setPeriod(shift(period, +1))}
